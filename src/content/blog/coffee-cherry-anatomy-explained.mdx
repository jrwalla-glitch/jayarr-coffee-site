...
<ReferenceGuide
  ...
  items={[
    ...
    {
      label: "6. Seed (endosperm + embryo)",
      description: "Two flat-faced seeds per cherry — mostly endosperm (the food-storage tissue we roast and brew) wrapped around a small embryo. After roasting, ~28-30% of the bean is water-soluble. Contains all flavor precursor compounds.",
      value: "Peaberry: single round seed, 5-10% of harvest."
    }
  ]}
...
The seed itself -- what the world calls a coffee "bean" -- is mostly **endosperm**: the starchy, oil- and protein-rich food-storage tissue that wraps a small **embryo** (the hypocotyl plus two cotyledons, a few millimeters long). When you roast and brew, you are extracting from the endosperm; the embryo is structurally inside it but contributes essentially nothing to the cup. Most cherries contain two seeds lying flat against each other with their flat sides facing inward. Occasionally a cherry produces only one seed, which develops into a round shape called a **peaberry** (about 5-10% of a harvest). Tanzanian peaberry lots are specifically sought after by some roasters who believe the rounder shape produces more even roasting.

After roasting, the bean is roughly 28-30% water-soluble by weight (the basis of the SCA brew control chart's 18-22% extraction yield window). Green coffee is far less soluble than that — roasting is what cracks open the cell structure and converts polysaccharides, sucrose, and chlorogenic acids into the soluble compounds we extract. The rest of the bean is insoluble cellulose that forms the rigid cell structure. Within those cells are the precursor compounds that roasting will transform into flavor:
